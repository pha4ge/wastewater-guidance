...
1. **Point-prevalence vs longitudinal prevalence**  
   ...
   Monitoring trends over time, and therefore sampling routinely, is helpful for determining the amount of a particular pathogen circulating in the community, as well as certain genotypes of that pathogen. This is commonly applied to seasonal pathogens, such as SARS-CoV-2, Influenza, and RSV. It can also be helpful for evaluating the impact intervention measures on community transmission of a given pathogen. Point prevalence studies, or those simply detecting the presence or absence of certain pathogens or genes, may include studies attempting to characterize the resistome of wastewater samples ([Huijbers et al, 2020](https://www.sciencedirect.com/science/article/pii/S0269749119363195);[Talat et al, 2023](https://journals.asm.org/doi/full/10.1128/spectrum.04102-22)). They can also be used to assess the efficacy of mitigation or disinfection efforts, including studies measuring abundance of pathogens or genes during different stages of the wastewater treatment process ([Che et al, 2019](https://link.springer.com/article/10.1186/s40168-019-0663-0); [Rodriguez et al, 2021](https://www.sciencedirect.com/science/article/abs/pii/S026974912100316X)).  
...
Wastewater sequencing is most commonly performed using three different approaches:
   ...
   * **Amplicon-based** \- amplicon sequencing targets specific regions of the genome of a particular pathogen. “Amplicons” refer to those regions of the genome that are amplified by the primers. This is a very common approach to wastewater sequencing of SARS-CoV-2, and is now being used for a much broader range of pathogens. Community-developed and validated amplicon schemes are now more commonly provided via shared public repositories, including through [primalscheme labs](https://labs.primalscheme.com/). These can then be synthesized and pooled for laboratory use. Commercially available amplicon schemes can also be an option for select pathogens. 

   * **Hybrid capture** \- a hybrid capture approach uses probes to capture specific genomic regions for sequencing. This is often applied to groups of microbes, such as respiratory pathogens, or the human virome ([Kantor et al, 2024](https://pubs.acs.org/doi/full/10.1021/acs.est.4c02638)). Among the most commonly used hybrid capture schemes are the [Illumina Virus Surveillance Panel (VSP)](https://www.illumina.com/products/by-type/sequencing-kits/library-prep-kits/viral-surveillance-panel.html) and the [Twist Comprehensive Viral Research Panel](https://www.twistbioscience.com/products/ngs/fixed-panels/comprehensive-viral-research-panel). 

Alternative approaches are currently in active development including modified metagenomic approaches, such as metagenomic sequencing with [spiked primer enrichment](https://www.biorxiv.org/content/10.64898/2025.12.26.690930v1). 

We detail the leading advantages and drawbacks of each of the three main sequencing approaches below:

| Approach | Primary use | Sensitivity to target (rare/low-prevalence detection) | Coverage of microbial diversity | Relative cost & sequencing depth needed |
| :---- | :---- | :---- | :---- | :---- |
| **Metagenomics (shotgun)** | Broad, untargeted characterization of the wastewater microbial/viral community. This approach is geared towards detection of unexpected pathogens, resistome/microbiome profiling, and situations where the target(s) of interest are not predefined. Poorly suited to tracking a specific low-prevalence target. | **Low.** Reads are distributed across the entire community, so a low-abundance target can be missed unless sequencing depth is very high. | **High.** Untargeted approach captures the full breadth of organisms present in a sample. | **High cost, high depth required.** Substantial sequencing depth (and compute) is needed to achieve adequate coverage of any single low-abundance target. |
| **Amplicon-based** | Tracking specific, high-priority, known targets (e.g., a defined pathogen or set of lineages/variants) and is well suited to detection at low prevalence/low concentration. | **High.** Targeted amplification concentrates sequencing effort on the target, enabling detection even at low concentrations. | **Low.** Restricted to the specific genome(s)/region(s) covered by the primer scheme; not useful for characterizing untargeted diversity. | **Low cost, low depth required.** Because reads are concentrated on the target, far less total sequencing depth is needed per sample. |
| **Hybrid capture** | Monitoring a defined panel of priority pathogens simultaneously, providing a middle ground between broad discovery and single-target tracking. | **Medium-High.** Probe-based enrichment improves sensitivity relative to metagenomics, though significantly less sensitive than a single-target amplicon scheme. For highly diverse pathogens, it provide additional robustness over amplicon sequencing approaches. | **Medium.** Covers a broader panel of organisms than amplicon sequencing, but still limited to what the probe set targets. | **Medium cost, medium depth required.** Enrichment reduces the depth needed relative to shotgun metagenomics, but panel size and probe synthesis add cost relative to amplicon approaches. |
...
## Minimum viable WES implementation

In many settings, full end-to-end wastewater and environmental surveillance (WES) may not be immediately feasible due to constraints in laboratory capacity, workforce, or infrastructure. To support early adoption while maintaining public health utility, programs may begin with a **minimum viable WES implementation** that prioritizes generation of reliable, actionable signals.

At minimum, programs should aim to include:

- **Population-representative sampling** from an accessible catchment (e.g., treatment plant influent or major sewer node)
- **Consistent sampling** generally at least once per week at each site
- **Validated pathogen detection assay** (e.g., RT-qPCR), with fecal indicator control (e.g., PMMoV)
- **Essential contextual metadata capture** (sampling location, date, method)
- **Defined data-to-action pathway** to translate findings to public health stakeholders

These components represent the core requirements for producing interpretable presence/absence or trend information. Additional elements such as genomic sequencing, high-frequency sampling, and real-time dashboards—can be incorporated progressively with growing technical expertise and additional funding.
